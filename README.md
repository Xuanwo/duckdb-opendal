...
This extension integrates Apache OpenDAL as a DuckDB filesystem implementation, enabling DuckDB to read files from supported storage backends via URI schemes.
...
./build/release/extension/opendal/opendal.duckdb_extension
```
- `duckdb` is the binary for the duckdb shell with the extension code automatically loaded.
- `unittest` is the test runner of duckdb. Again, the extension is already linked into the binary.
- `opendal.duckdb_extension` is the loadable binary as it would be distributed.
...
Then read a local file via OpenDAL fs:
```
D SELECT * FROM 'opendal+fs://path/to/file.csv';
```
...
## Formatting
This repo uses `uv` to manage the Python tooling required by `duckdb/scripts/format.py`.
